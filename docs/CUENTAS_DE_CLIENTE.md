...
SQL Editor del proyecto → ejecutar `supabase/migrations/0008_auth_identity_sync.sql`.
...
Ojo: `orders.customer_id` referencia `profiles(id)`, que a su vez cascadea
desde `auth.users`. **Borrar un invitado borra su perfil y rompe la
referencia de sus pedidos.** Antes de correr esto en producción hay que
decidir qué pasa con esos pedidos (conservarlos exige aflojar la FK o
mover el pedido a un perfil de archivo). Mientras esa decisión no esté
tomada, **no ejecutarlo**.
