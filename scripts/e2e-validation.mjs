...
try {
  ...
  await run("carrito persistente y checkout completo", async () => {
    ...
    await page.getByText("Cliente Prueba").waitFor();
    await page.locator(".order-card").waitFor();
    await context.close();
  });

  await run("registro, login y protección de administración", async () => {
