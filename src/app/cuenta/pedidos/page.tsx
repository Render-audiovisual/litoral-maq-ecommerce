      ...
      <div className="account-layout">
        ...
        <section>
          ...
          {!ownOrders.length ? <div className="empty-state"><span>▤</span><h2>Todavía no tenés pedidos</h2><p>Cuando confirmes un pedido desde la tienda va a aparecer acá.</p><Link href="/productos" className="button primary">Explorar productos</Link></div> : (
            <div className="order-list">{ownOrders.map((order) => <article className="order-card" key={order.id}><div><span>Pedido</span><strong>{order.id}</strong><small>{formatDate(order.createdAt)}</small></div><div><span>Total</span><strong>{formatCurrency(order.total)}</strong><small>{order.lines.reduce((sum, line) => sum + line.quantity, 0)} unidades</small></div><div><span>Entrega</span><strong>{order.deliveryMethod === "envio" ? "Envío" : "Retiro"}</strong><small>{order.address || "Sáenz 1587"}</small></div><div><span className={`status status-${order.status}`}>{ORDER_STATUS_LABELS[order.status]}</span><small>{order.paymentReference}</small></div><details className="customer-order-lines"><summary>Ver productos</summary>{resolveOrderLines(order, products).map((line) => <div key={`${line.productId}-${line.productCode}`}><span>{line.quantity} × {line.productName}</span><strong>{formatCurrency(line.lineTotal)}</strong></div>)}</details></article>)}</div>
          )}
        </section>
      </div>
    </main>
