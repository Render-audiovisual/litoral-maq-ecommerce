...
import { mockSheetSyncAdapter } from "@/services/mock";
...
export default function AdminProductsPage() {
  ...
  function submit(event: FormEvent) {
    ...
    setEditing(null); setMessage("Producto guardado correctamente.");
  }

  async function sync() {
    setSyncing(true); const preview = await mockSheetSyncAdapter.preview(); replaceProducts(preview.products);
    const result = await mockSheetSyncAdapter.sync(); setMessage(`Sincronización demo completa: ${result.updated} productos actualizados.`); setSyncing(false);
  }

  return (
    <main className="admin-content">
      <div className="admin-heading"><div><span className="eyebrow orange">CATÁLOGO</span><h1>Productos</h1><p>{products.length} productos · 460 provenientes del Google Sheet</p></div><div className="heading-actions"><button type="button" className="button secondary" onClick={sync} disabled={syncing}>{syncing ? "Sincronizando…" : "↻ Actualizar desde Sheet"}</button><button type="button" className="button primary" onClick={() => setEditing(emptyProduct())}>+ Nuevo producto</button></div></div>
      {message && <div className="success-message dismissible">{message}<button onClick={() => setMessage("")}>×</button></div>}
