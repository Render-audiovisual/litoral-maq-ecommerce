...
function SuccessContent() {
  ...
  return (
    <main className="center-state success-page">
      ...
      <p>{paymentEnabled ? approved ? (order?.deliveryMethod === "retiro" ? "Mercado Pago confirmó el pago. Preparamos tu pedido y te avisamos cuando puedas retirarlo en el local de Sáenz 1587." : order && isShippingToCoordinate(order) ? `Mercado Pago confirmó el pago de los productos. Litoral Maq te va a contactar para coordinar el envío${order.shippingCarrier ? ` con ${order.shippingCarrier}` : ""} y su costo.` : "Mercado Pago confirmó el pago. Preparamos tu pedido y te enviamos el seguimiento cuando se despache.") : "La vuelta desde Mercado Pago no aprueba el pedido por sí sola. El estado se actualizará cuando recibamos la notificación firmada." : "Todavía no se realizó ningún cobro. Vamos a confirmar disponibilidad, entrega y total final antes de coordinar el pago."}</p>
      <div className="whatsapp-confirmation"><p>{approved ? (order && isShippingToCoordinate(order) ? "Coordiná el envío con nosotros" : "¿Tenés alguna consulta?") : "¿Querés acelerar la confirmación?"}</p><a href={getOrderWhatsAppUrl(order, orderId)} className="button whatsapp-button" target="_blank" rel="noopener noreferrer">{approved ? "Escribirnos por WhatsApp" : "Avisar por WhatsApp"}</a><small>Se abrirá un mensaje con el número de solicitud y sus productos.</small></div>
      {hasAccount ? (
