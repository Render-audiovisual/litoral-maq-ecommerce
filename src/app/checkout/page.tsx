...
export default function CheckoutPage() {
  ...
  const { cart, cartSubtotal, session, clearCart, createOrder, addCustomer, setSession } = useStore();
  ...
  async function submit(event: FormEvent) {
    ...
    const order: Order = {
      id,
      customerId: session?.user.id || `guest-${form.email}`,
    ...
    if (!session) {
      setSession({
        ...
        token: `checkout-demo-${Date.now()}`,
      });
    }
    clearCart();
    router.push(`/checkout/exito?pedido=${order.id}`);
  }
