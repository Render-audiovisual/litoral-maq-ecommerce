...
describe("supabase persistence adapter", () => {
  ...
  it("updateProduct devuelve null si ninguna fila coincidi� (otra persona lo cambi�)", async () => {
    const { client } = createFakeClient({ products: { data: [], error: null } });
    const adapter = createSupabasePersistenceAdapter(client);
    expect(await adapter.updateProduct("p1", { featured: true }, "v1")).toBeNull();
  });

  it("updateOrderPaymentStatus exige el pago que ve�a la persona", async () => {
